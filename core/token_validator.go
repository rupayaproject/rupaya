// Copyright (c) 2018 // Copyright (c) 2018 Tomochain
//                                   (c) 2020 Rupaya
//
...
func ValidateRupXApplyTransaction(chain consensus.ChainContext, blockNumber *big.Int, copyState *state.StateDB, tokenAddr common.Address) error {
	if blockNumber == nil || blockNumber.Sign() <= 0 {
		blockNumber = chain.CurrentHeader().Number
	}
	if !chain.Config().IsRIPRupX(blockNumber) {
		return nil
	}
	contractABI, err := GetTokenAbi(contract.RRC21ABI)
	if err != nil {
		return fmt.Errorf("ValidateRupXApplyTransaction: cannot parse ABI. Err: %v", err)
	}
...
func ValidateRupZApplyTransaction(chain consensus.ChainContext, blockNumber *big.Int, copyState *state.StateDB, tokenAddr common.Address) error {
	if blockNumber == nil || blockNumber.Sign() <= 0 {
		blockNumber = chain.CurrentHeader().Number
	}
	if !chain.Config().IsRIPRupX(blockNumber) {
	...
	contractABI, err := GetTokenAbi(contract.RRC21ABI)
	if err != nil {
		return fmt.Errorf("ValidateRupZApplyTransaction: cannot parse ABI. Err: %v", err)
	}
	// verify balance slot
	if err := ValidateBalanceSlot(chain, copyState, tokenAddr, contractABI); err != nil {
		return err
...
func SetRandomBalance(copyState *state.StateDB, tokenAddr, addr common.Address, randomValue *big.Int) {
	slotBalanceRrc21 := state.SlotRRC21Token["balances"]
	balanceKey := state.GetLocMappingAtKey(addr.Hash(), slotBalanceRrc21)
...
func ValidateMinFeeSlot(chain consensus.ChainContext, copyState *state.StateDB, tokenAddr common.Address, contractABI *abi.ABI) error {
	...
	slotMinFeeRrc21 := state.SlotRRC21Token["minFee"]
	copyState.SetState(tokenAddr, common.BigToHash(new(big.Int).SetUint64(slotMinFeeRrc21)), common.BytesToHash(randomValue.Bytes()))

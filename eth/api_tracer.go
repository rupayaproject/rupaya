...
import (
	...
	"github.com/rupayaproject/go-rupaya/eth/tracers"
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				...
				feeCapacity := state.GetRRC21FeeCapacityFromState(task.statedb)
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	...
	feeCapacity := state.GetRRC21FeeCapacityFromState(statedb)
	if common.RIPSigning.Cmp(block.Header().Number) == 0 {

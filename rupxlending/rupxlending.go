...
func New(rupx *rupx.RupX) *Lending {
	...
	lending.StateCache = lendingstate.NewDatabase(rupx.GetLevelDB())
	lending.rupx = rupx
	return lending
}

func (l *Lending) GetLevelDB() rupxDAO.RupXDAO {
	return l.rupx.GetLevelDB()
}

func (l *Lending) GetMongoDB() rupxDAO.RupXDAO {
	return l.rupx.GetMongoDB()
}
	...
	return []rpc.API{
		{
			...
			Service:   NewPublicRupXLendingAPI(l),
			Public:    true,
		},
	}
}
...
func (l *Lending) SyncDataToSDKNode(chain consensus.ChainContext, statedb *state.StateDB, block *types.Block, takerLendingItem *lendingstate.LendingItem, txHash common.Hash, txMatchTime time.Time, trades []*lendingstate.LendingTrade, rejectedItems []*lendingstate.LendingItem, dirtyOrderCount *uint64) error {
	...
	if takerLendingItem.Status == lendingstate.LendingStatusNew {
		...
		if updatedTakerLendingItem.Side == lendingstate.Borrowing && updatedTakerLendingItem.CollateralToken.String() != lendingstate.EmptyAddress {
			tradingStateDb, _ := l.rupx.GetTradingState(block)
			...
			collateralTokenDecimal, err = l.rupx.GetTokenDecimal(chain, statedb, updatedTakerLendingItem.CollateralToken)
...
func (l *Lending) RollbackLendingData(txhash common.Hash) error {
	...
	if items != nil {
		for _, item := range items.([]*lendingstate.LendingItem) {
			c, ok := l.lendingItemHistory.Get(txhash)
			log.Debug("rupxlending reorg: rollback lendingItem", "txhash", txhash.Hex(), "item", lendingstate.ToJSON(item), "lendingItemHistory", c)
			if !ok {
				log.Debug("rupxlending reorg: remove item due to no lendingItemHistory", "item", lendingstate.ToJSON(item))
			...
			if (lendingItemHistory == lendingstate.LendingItemHistoryItem{}) {
				log.Debug("rupxlending reorg: remove item due to empty lendingItemHistory", "item", lendingstate.ToJSON(item))
				if err := db.DeleteObject(item.Hash, &lendingstate.LendingItem{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingItem. Err: %v . Item: %s", err.Error(), lendingstate.ToJSON(item))
				}
				continue
			...
			log.Debug("rupxlending reorg: update item to the last lendingItemHistory", "item", lendingstate.ToJSON(item), "lendingItemHistory", lendingItemHistory)
	...
	if items != nil {
		for _, trade := range items.([]*lendingstate.LendingTrade) {
			c, ok := l.lendingTradeHistory.Get(txhash)
			log.Debug("rupxlending reorg: rollback LendingTrade", "txhash", txhash.Hex(), "trade", lendingstate.ToJSON(trade), "LendingTradeHistory", c)
			if !ok {
				log.Debug("rupxlending reorg: remove trade due to no LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
			...
			if (lendingTradeHistoryItem == lendingstate.LendingTradeHistoryItem{}) {
				log.Debug("rupxlending reorg: remove trade due to empty LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
				if err := db.DeleteObject(trade.Hash, &lendingstate.LendingTrade{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingTrade. Err: %v . Trade: %s", err.Error(), lendingstate.ToJSON(trade))
				}
				continue
			...
			log.Debug("rupxlending reorg: update trade to the last lendingTradeHistoryItem", "trade", lendingstate.ToJSON(trade), "lendingTradeHistoryItem", lendingTradeHistoryItem)
			if err := db.PutObject(trade.Hash, trade); err != nil {

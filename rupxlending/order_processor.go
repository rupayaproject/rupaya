...
func (l *Lending) processOrderList(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, lendingStateDB *lendingstate.LendingStateDB, tradingStateDb *tradingstate.TradingStateDB, side string, lendingOrderBook common.Hash, Interest *big.Int, quantityStillToTrade *big.Int, order *lendingstate.LendingItem) (*big.Int, []*lendingstate.LendingTrade, []*lendingstate.LendingItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		tradedQuantity, collateralLockedAmount, rejectMaker, settleBalanceResult, err := l.getLendQuantity(lendTokenRUPXPrice, collateralPrice, depositRate, borrowFee, coinbase, chain, statedb, order, &oldestOrder, maxTradedQuantity)
...
func (l *Lending) getLendQuantity(
	...
	LendingTokenDecimal, err := l.rupx.GetTokenDecimal(chain, statedb, makerOrder.LendingToken)
	...
	collateralTokenDecimal, err := l.rupx.GetTokenDecimal(chain, statedb, collateralToken)
	...
	if quantity.Sign() > 0 {
		// Apply Match Order
		settleBalanceResult, err := lendingstate.GetSettleBalance(takerOrder.Side, lendTokenRUPXPrice, collateralPrice, depositRate, borrowFee, lendToken, collateralToken, LendingTokenDecimal, collateralTokenDecimal, quantity)
...
func (l *Lending) ProcessCancelOrder(header *types.Header, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, chain consensus.ChainContext, coinbase common.Address, lendingOrderBook common.Hash, order *lendingstate.LendingItem) (error, bool) {
	...
	lendTokenDecimal, err := l.rupx.GetTokenDecimal(chain, statedb, originOrder.LendingToken)
	...
	if originOrder.Side == lendingstate.Borrowing {
		...
		collateralTokenDecimal, err = l.rupx.GetTokenDecimal(chain, statedb, originOrder.CollateralToken)
...
func (l *Lending) GetMediumTradePriceBeforeEpoch(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, baseToken common.Address, quoteToken common.Address) (*big.Int, error) {
	...
	if price != nil && price.Sign() > 0 {
		...
		if inversePrice != nil && inversePrice.Sign() > 0 {
			quoteTokenDecimal, err := l.rupx.GetTokenDecimal(chain, statedb, quoteToken)
			if err != nil || quoteTokenDecimal.Sign() == 0 {
				return nil, fmt.Errorf("Fail to get tokenDecimal. Token: %v . Err: %v", quoteToken.String(), err)
			}
			baseTokenDecimal, err := l.rupx.GetTokenDecimal(chain, statedb, baseToken)
...
//- Have direct pair in RupX: lendToken/CollateralToken or CollateralToken/LendToken
...
func (l *Lending) GetCollateralPrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, collateralToken common.Address, lendingToken common.Address) (*big.Int, *big.Int, error) {
	...
	lendingTokenDecimal, err := l.rupx.GetTokenDecimal(chain, statedb, lendingToken)
	...
	// getting price from pair in rupx
	...
	if lastAveragePrice != nil && lastAveragePrice.Sign() > 0 {
		log.Debug("Getting collateral/lending from direct pair in rupx", "lendToken", lendingToken.Hex(), "collateralToken", collateralToken.Hex(), "price", lastAveragePrice)
...
func (l *Lending) GetRUPXBasePrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, token common.Address) (*big.Int, error) {
	...
	if token == common.HexToAddress(common.RupayaNativeAddress) {
		...
		// otherwise, getting from rupx lendToken/RUPX
		log.Debug("Getting token/RUPX price from contract", "price", tokenRUPXPriceFromContract)
		return tokenRUPXPriceFromContract, nil
	} else {
		rupxTokenPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, common.HexToAddress(common.RupayaNativeAddress), token)
		rupxTokenPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().Posv.Epoch == header.Number.Uint64()/chain.Config().Posv.Epoch
		if rupxTokenPriceUpdatedFromContract && rupxTokenPriceFromContract != nil && rupxTokenPriceFromContract.Sign() > 0 {
			// getting lendToken price from contract first
			// otherwise, getting from rupx lendToken/RUPX
			log.Debug("Getting RUPX/token from contract", "price", rupxTokenPriceFromContract)
			tokenDecimal, err := l.rupx.GetTokenDecimal(chain, statedb, token)
			log.Debug("GetTokenDecimal", "token", token.Hex(), "err", err)
			if err != nil || tokenDecimal == nil || tokenDecimal.Sign() == 0 {
				return nil, err
			}
			tokenRupxPrice := new(big.Int).Mul(common.BasePrice, tokenDecimal)
			tokenRupxPrice = new(big.Int).Div(tokenRupxPrice, rupxTokenPriceFromContract)
		...
		if tokenRUPXPrice != nil && tokenRUPXPrice.Sign() > 0 {
			log.Debug("Getting token/RUPX from rupx", "price", tokenRUPXPrice, "err", err)
			return tokenRUPXPrice, nil
		}
	}

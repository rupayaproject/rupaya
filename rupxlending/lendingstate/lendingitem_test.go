	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := VerifyBalance(statedb,
				...
				EtherToWei(big.NewInt(1)),
				EtherToWei(big.NewInt(2)),    // RUPX price: 0.5 USD => USD/RUPX = 2
...
func sendOrder(nonce uint64) {
	...
	err = rpcClient.Call(&result, "rupx_sendLending", item)
	fmt.Println("sendLendingitem", "nonce", item.AccountNonce)
	if err != nil {
		fmt.Println("rpcClient.Call rupx_sendLending failed", "err", err)
		os.Exit(1)
	}
}

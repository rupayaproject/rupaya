...
func (t *Trade) SetBSON(raw bson.Raw) error {
	...
	t.Amount = rupx_state.ToBigInt(decoded.Amount)
	t.PricePoint = rupx_state.ToBigInt(decoded.PricePoint)

	t.MakeFee = rupx_state.ToBigInt(decoded.MakeFee)
	t.TakeFee = rupx_state.ToBigInt(decoded.TakeFee)

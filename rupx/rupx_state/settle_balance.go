...
func GetSettleBalance(quotePrice *big.Int, takerSide string, takerFeeRate *big.Int, baseToken, quoteToken common.Address, makerPrice *big.Int, makerFeeRate *big.Int, baseTokenDecimal *big.Int, quoteTokenDecimal *big.Int, quantityToTrade *big.Int) (*SettleBalance, error) {
	...
	if takerSide == Bid {
		...
		if baseToken.String() != common.RupayaNativeAddress && quotePrice != nil && quotePrice.Cmp(common.Big0) > 0 {
		...
		} else if baseToken.String() == common.RupayaNativeAddress {
			exMakerReceivedFee := new(big.Int).Mul(quantityToTrade, makerFeeRate)
			exMakerReceivedFee = exMakerReceivedFee.Div(exMakerReceivedFee, common.RupXBaseFee)
			...
			exTakerReceivedFee = exTakerReceivedFee.Div(exTakerReceivedFee, common.RupXBaseFee)

...
func (db *MongoDatabase) GetObject(hash common.Hash, val interface{}) (interface{}, error) {
	...
	if cached, ok := db.cacheItems.Get(cacheKey); ok {
		...
		switch val.(type) {
		case *rupx_state.OrderItem:
			var oi *rupx_state.OrderItem
...
func (db *MongoDatabase) CommitOrder(o *rupx_state.OrderItem) error {
...
func (db *MongoDatabase) GetOrderByTxHash(txhash common.Hash) []*rupx_state.OrderItem {
	var result []*rupx_state.OrderItem
	sc := db.Session.Copy()
	defer sc.Close()
...
func (db *MongoDatabase) GetListOrderByHashes(hashes []string) []*rupx_state.OrderItem {
	var result []*rupx_state.OrderItem
	sc := db.Session.Copy()
	defer sc.Close()

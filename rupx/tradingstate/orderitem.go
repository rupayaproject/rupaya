...
func VerifyBalance(statedb *state.StateDB, rupxStakeDb *TradingStateDB, order *types.OrderTransaction, baseDecimal, quoteDecimal *big.Int) error {
	var quotePrice *big.Int
	if order.QuoteToken().String() != common.RupayaNativeAddress {
		quotePrice = rupxStakeDb.GetLastPrice(GetTradingOrderBookHash(order.QuoteToken(), common.HexToAddress(common.RupayaNativeAddress)))
		log.Debug("TryGet quotePrice QuoteToken/RUPX", "quotePrice", quotePrice)
		if quotePrice == nil || quotePrice.Sign() == 0 {
			inversePrice := rupxStakeDb.GetLastPrice(GetTradingOrderBookHash(common.HexToAddress(common.RupayaNativeAddress), order.QuoteToken()))
			...
			if inversePrice != nil && inversePrice.Sign() > 0 {
				...
				log.Debug("TryGet quotePrice after get inversePrice RUPX/QuoteToken", "quotePrice", quotePrice, "quoteTokenDecimal", quoteDecimal)
			}
		}
	} else {

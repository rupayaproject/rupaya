...
const DefaultFeeRate = 10 // 10 / RupXBaseFee = 10 / 10000 = 0.1%
...
func GetSettleBalance(quotePrice *big.Int, takerSide string, takerFeeRate *big.Int, baseToken, quoteToken common.Address, makerPrice *big.Int, makerFeeRate *big.Int, baseTokenDecimal *big.Int, quoteTokenDecimal *big.Int, quantityToTrade *big.Int) (*SettleBalance, error) {
	...
	if takerSide == Bid {
		...
		if quoteToken.String() != common.RupayaNativeAddress && quotePrice != nil && quotePrice.Cmp(common.Big0) > 0 {
			// defaultFeeInRUPX
			defaultFeeInRUPX := new(big.Int).Mul(defaultFee, quotePrice)
			defaultFeeInRUPX = new(big.Int).Div(defaultFeeInRUPX, quoteTokenDecimal)
			...
			if (exMakerReceivedFee.Cmp(common.RelayerFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInRUPX.Cmp(common.RelayerFee) <= 0 {
				log.Debug("makerFee too small", "quoteTokenQuantity", quoteTokenQuantity, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "quotePrice", quotePrice, "defaultFeeInRUPX", defaultFeeInRUPX)
				return result, ErrQuantityTradeTooSmall
			}
			...
			if (exTakerReceivedFee.Cmp(common.RelayerFee) <= 0 && exTakerReceivedFee.Sign() > 0) || defaultFeeInRUPX.Cmp(common.RelayerFee) <= 0 {
				log.Debug("takerFee too small", "quoteTokenQuantity", quoteTokenQuantity, "takerFee", takerFee, "exTakerReceivedFee", exTakerReceivedFee, "quotePrice", quotePrice, "defaultFeeInRUPX", defaultFeeInRUPX)
				return result, ErrQuantityTradeTooSmall
			}
		} else if quoteToken.String() == common.RupayaNativeAddress {
		...
		if quoteToken.String() != common.RupayaNativeAddress && quotePrice != nil && quotePrice.Cmp(common.Big0) > 0 {
			...
			if (exMakerReceivedFee.Cmp(common.RelayerFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInRUPX.Cmp(common.RelayerFee) <= 0 {
				log.Debug("makerFee too small", "quoteTokenQuantity", quoteTokenQuantity, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "quotePrice", quotePrice, "defaultMakerFeeInRUPX", defaultFeeInRUPX)
				return result, ErrQuantityTradeTooSmall
			}
			exTakerReceivedFee := new(big.Int).Mul(takerFee, quotePrice)
			exTakerReceivedFee = new(big.Int).Div(exTakerReceivedFee, quoteTokenDecimal)

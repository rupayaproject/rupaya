...
func (rupx *RupX) CommitOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	rupxSnap := tradingStateDB.Snapshot()
	dbSnap := statedb.Snapshot()
	trades, rejects, err := rupx.ApplyOrder(coinbase, chain, statedb, tradingStateDB, orderBook, order)
	if err != nil {
		tradingStateDB.RevertToSnapshot(rupxSnap)
...
func (rupx *RupX) ApplyOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
	...
	if order.Status == tradingstate.OrderStatusCancelled {
		err, reject := rupx.ProcessCancelOrder(tradingStateDB, statedb, chain, coinbase, orderBook, order)
		if err != nil || reject {
	...
	if orderType == tradingstate.Market {
		...
		trades, rejects, err = rupx.processMarketOrder(coinbase, chain, statedb, tradingStateDB, orderBook, order)
		if err != nil {
		...
		trades, rejects, err = rupx.processLimitOrder(coinbase, chain, statedb, tradingStateDB, orderBook, order)
		if err != nil {
...
func (rupx *RupX) processMarketOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
		trades     []map[string]string
	...
	if side == tradingstate.Bid {
		...
		for quantityToTrade.Cmp(zero) > 0 && bestPrice.Cmp(zero) > 0 {
			quantityToTrade, newTrades, newRejects, err = rupx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Ask, orderBook, bestPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
		...
		log.Debug("processMarketOrder ", "side", side, "bestPrice", bestPrice, "quantityToTrade", quantityToTrade, "volume", volume)
		for quantityToTrade.Cmp(zero) > 0 && bestPrice.Cmp(zero) > 0 {
			quantityToTrade, newTrades, newRejects, err = rupx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Bid, orderBook, bestPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
...
func (rupx *RupX) processLimitOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
		trades     []map[string]string
		newTrades  []map[string]string
		rejects    []*tradingstate.OrderItem
	...
	if side == tradingstate.Bid {
		...
		for quantityToTrade.Cmp(zero) > 0 && price.Cmp(minPrice) >= 0 && minPrice.Cmp(zero) > 0 {
			...
			quantityToTrade, newTrades, newRejects, err = rupx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Ask, orderBook, minPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
		...
		for quantityToTrade.Cmp(zero) > 0 && price.Cmp(maxPrice) <= 0 && maxPrice.Cmp(zero) > 0 {
			...
			quantityToTrade, newTrades, newRejects, err = rupx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Bid, orderBook, maxPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
...
func (rupx *RupX) processOrderList(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, side string, orderBook common.Hash, price *big.Int, quantityStillToTrade *big.Int, order *tradingstate.OrderItem) (*big.Int, []map[string]string, []*tradingstate.OrderItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		if oldestOrder.QuoteToken.String() != common.RupayaNativeAddress {
			quotePrice = tradingStateDB.GetLastPrice(tradingstate.GetTradingOrderBookHash(oldestOrder.QuoteToken, common.HexToAddress(common.RupayaNativeAddress)))
			log.Debug("TryGet quotePrice QuoteToken/RUPX", "quotePrice", quotePrice)
			if quotePrice == nil || quotePrice.Sign() == 0 {
				inversePrice := tradingStateDB.GetLastPrice(tradingstate.GetTradingOrderBookHash(common.HexToAddress(common.RupayaNativeAddress), oldestOrder.QuoteToken))
				quoteTokenDecimal, err := rupx.GetTokenDecimal(chain, statedb, oldestOrder.QuoteToken)
...
func (rupx *RupX) getTradeQuantity(quotePrice *big.Int, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, takerOrder *tradingstate.OrderItem, makerOrder *tradingstate.OrderItem, quantityToTrade *big.Int) (*big.Int, bool, error) {
	baseTokenDecimal, err := rupx.GetTokenDecimal(chain, statedb, makerOrder.BaseToken)
	...
	quoteTokenDecimal, err := rupx.GetTokenDecimal(chain, statedb, makerOrder.QuoteToken)
	if err != nil || quoteTokenDecimal.Sign() == 0 {
...
func GetTradeQuantity(takerSide string, takerFeeRate *big.Int, takerBalance *big.Int, makerPrice *big.Int, makerFeeRate *big.Int, makerBalance *big.Int, baseTokenDecimal *big.Int, quantityToTrade *big.Int) (*big.Int, bool) {
	if takerSide == tradingstate.Bid {
		...
		if takerBalance.Cmp(takerOutTotal) >= 0 && makerBalance.Cmp(makerOutTotal) >= 0 {
			...
			newQuantityTrade = new(big.Int).Mul(newQuantityTrade, common.RupXBaseFee)
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, new(big.Int).Add(common.RupXBaseFee, takerFeeRate))
		...
		if takerBalance.Cmp(takerOutTotal) >= 0 && makerBalance.Cmp(makerOutTotal) >= 0 {
			...
			newQuantityTrade = new(big.Int).Mul(newQuantityTrade, common.RupXBaseFee)
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, new(big.Int).Add(common.RupXBaseFee, makerFeeRate))
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, makerPrice)
			log.Debug("Reject order maker , not enough balance ", "makerBalance", makerBalance, " makerOutTotal", makerOutTotal)
...
func (rupx *RupX) ProcessCancelOrder(tradingStateDB *tradingstate.TradingStateDB, statedb *state.StateDB, chain consensus.ChainContext, coinbase common.Address, orderBook common.Hash, order *tradingstate.OrderItem) (error, bool) {
	...
	baseTokenDecimal, err := rupx.GetTokenDecimal(chain, statedb, order.BaseToken)
	if err != nil || baseTokenDecimal.Sign() == 0 {
...
func (rupx *RupX) UpdateMediumPriceBeforeEpoch(epochNumber uint64, tradingStateDB *tradingstate.TradingStateDB, statedb *state.StateDB) error {
	...
	if rupx.IsSDKNode() {
		if err := rupx.LogEpochPrice(epochNumber, epochPriceResult); err != nil {
...
// orderbook hash genereted from baseToken, quoteToken at rupaya/rupx/tradingstate/common.go:214
func (rupx *RupX) LogEpochPrice(epochNumber uint64, epochPriceResult map[common.Hash]*big.Int) error {
	db := rupx.GetMongoDB()
	db.InitBulk()
